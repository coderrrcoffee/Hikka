...
class Web:
    ...
    async def okteto(self, request):
        if any(cl[2].get("OwO", "okteto_uri", False) for cl in self.client_data.values()):
            return web.Response(status=418)

        text = await request.text()

        for client_uid in self.client_data:
            self.client_data[client_uid][2].set("OwO", "okteto_uri", text)
    ...
    async def finish_login(self, request):
        ...
        first_session = not bool(main.OwO.clients)

        await main.OwO.fetch_clients_from_web()
        ...
        if not first_session:
            ...
            for client in main.OwO.clients:
    ...
    async def web_auth(self, request):
        ...
        if not await main.OwO.wait_for_web_auth(token):
