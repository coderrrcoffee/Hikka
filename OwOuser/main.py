    ...
    def _get_phones(self) -> None:
        ...
        phones = {
            ...
            for phone in map(
                lambda f: f[6:-8],
                filter(
                    lambda f: f.startswith("OwO-") and f.endswith(".session"),
                    os.listdir(
    ...
    async def fetch_clients_from_web(self) -> None:
        ...
        for client in self.web.clients:
            session = SQLiteSession(
                os.path.join(
                    self.arguments.data_root or DATA_DIR,
                    f"OwO-+{'x' * (len(client.phone) - 5)}{client.phone[-4:]}-{(await client.get_me()).id}",
                )
            )
    ...
    def _init_clients(self) -> None:
        ...
        for phone_id, phone in self.phones.items():
            session = os.path.join(
                self.arguments.data_root or DATA_DIR,
                f'OwO{f"-{phone_id}" if phone_id else ""}',
            )
            ...
            try:
                ...
                print(
                    ...
                    f"If that doesn't help, delete the file named 'OwO-{phone or ''}.session'"
                )
                continue
        ...
        try:
            ...
            if not omit_log:
                logging.info(
                    ...
                    f"OwOuserbot version: {'.'.join(list(map(str, list(__version__))))}\n"
