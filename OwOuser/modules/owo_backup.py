...
@loader.tds
class OwOBackupMod(loader.Module):
    ...
    async def handler(self) -> None:
        while True:
            try:
                ...
                backup.name = f"OwO-db-backup-{getattr(datetime, 'datetime', datetime).now().strftime('%d-%m-%Y-%H-%M')}.json"
                ...
                logger.exception("OwOBackup failed")
                await asyncio.sleep(3)
