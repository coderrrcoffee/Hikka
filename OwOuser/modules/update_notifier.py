...
class UpdateNotifierMod(loader.Module):
    """Tracks latest OwO releases, and notifies you, if update is required"""

    strings = {
        "name": "UpdateNotifier",
        "update_required": "🌘 <b>OwO Update available!</b>\n\nNew OwOuserbot version released.\n🔮 <b>OwO <s>{}</s> -> {}</b>\n\n{}",
        "more": "\n<i><b>🎥 And {} more...</b></i>",
    }
    ...
    async def client_ready(self, client, db) -> None:
        ...
        self._markup.add(
            InlineKeyboardButton(
                "🔄 Update",
                callback_data="OwO_update",
            )
        )
        self._markup.add(
            InlineKeyboardButton(
                "🚫 Ignore",
                callback_data="OwO_upd_ignore",
            )
        )
    ...
    async def update_callback_handler(self, call: CallbackQuery) -> None:
        ...
        @allow: sudo
        """
        if call.data not in {"OwO_update", "OwO_upd_ignore"}:
            return

        if call.data == "OwO_upd_ignore":
